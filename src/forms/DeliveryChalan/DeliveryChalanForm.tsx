  ...
  const handleSaveDeliveryChalan = async () => {
    ...
    try {
      ...
      if (isEditing && id) {
        ...
        await axios.put(`${API_DELIVERY_CHALANS}/${id}`, chalanData);
        chalanId = parseInt(id);
        
        // Delete existing delivery chalan items
        const existingItems = await axios.get(`${API_DELIVERY_CHALAN_ITEMS}?chalan_id=${chalanId}`);
        ...
        const response = await axios.post(API_DELIVERY_CHALANS, chalanData);
      ...
      for (const item of chalanItems) {
        ...
        await axios.post(API_DELIVERY_CHALAN_ITEMS, itemData);
      }
