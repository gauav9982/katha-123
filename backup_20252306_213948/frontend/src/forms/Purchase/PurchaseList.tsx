...
const PurchaseList: React.FC = () => {
  ...
  // Local state for purchases since store might not have it
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${API_URL.PURCHASES}/${selectedPurchase.id}`);
    ...
    try {
      ...
      if (!purchase.items || purchase.items.length === 0) {
        if (purchase.id) {
          ...
          const purchaseResponse = await axios.get(`${API_URL.PURCHASES}/${purchase.id}`);
          const itemsResponse = await axios.get(`${API_URL.PURCHASE_ITEMS}?purchase_id=${purchase.id}`);
  ...
  // Add defensive coding to handle undefined purchases
