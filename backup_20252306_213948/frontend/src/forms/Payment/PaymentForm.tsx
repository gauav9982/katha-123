...
const PaymentForm: React.FC = () => {
  ...
  const fetchPayment = async () => {
    ...
    try {
      ...
      try {
        const partyResponse = await axios.get(`${API_URL.PARTIES}/${response.data.party_id}`);
  ...
  const fetchPurchaseInvoices = async (partyId: number) => {
    try {
      ...
      try {
        const response = await axios.get(`${API_URL.PENDING_PURCHASES}?party_id=${partyId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!payment.party_id || !payment.amount) {
