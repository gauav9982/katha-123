...
const DeliveryChalanList: React.FC = () => {
  ...
  const fetchDeliveryChalans = async () => {
    setLoading(true);
    try {
      const res = await axios.get(API_URL.DELIVERY_CHALANS);
      setDeliveryChalans(res.data);
    } catch (error) {
  ...
  const handleDeleteDeliveryChalan = async (id: number) => {
    ...
    try {
      ...
      const itemsResponse = await axios.get(`${API_URL.DELIVERY_CHALAN_ITEMS}?chalan_id=${id}`);
      for (const item of itemsResponse.data) {
        await axios.delete(`${API_URL.DELIVERY_CHALAN_ITEMS}/${item.id}`);
      }
      
      // Then delete the delivery chalan
      await axios.delete(`${API_URL.DELIVERY_CHALANS}/${id}`);
