...
// API endpoints
const API_CREDITSALES = 'http://168.231.122.33:4000/api/creditsales';
const API_CREDITSALE_ITEMS = 'http://168.231.122.33:4000/api/creditsale-items';
...
const CreditSaleList: React.FC = () => {
  ...
  const loadCreditSales = async () => {
    ...
    try {
      ...
      setCreditSales(sortedData);
      setFilteredSales(sortedData);
      console.log(`Loaded ${sortedData.length} credit sales`);
    } catch (error) {
      console.error('Error loading credit sales:', error);
      showAlert('Failed to load credit sales from server', 'error');
    } finally {
      setLoading(false);
    }
  };

  // Filter credit sales when search query changes
  useEffect(() => {
    if (searchQuery.trim() === '') {
      setFilteredSales(creditSales);
    } else {
      const query = searchQuery.toLowerCase().trim();
      const filtered = creditSales.filter(
        (sale) =>
  ...
  // Handle delete credit sale
  const handleDelete = async (id: number) => {
    if (isDeleting) return;
    
    if (!window.confirm('Are you sure you want to delete this credit sale? This action cannot be undone.')) {
      return;
    }
    
    setIsDeleting(true);
    try {
      // First delete all credit sale items
      const itemsResponse = await axios.get(`${API_CREDITSALE_ITEMS}?sale_id=${id}`);
      ...
      await axios.delete(`${API_CREDITSALES}/${id}`);
      
      showAlert('Credit sale deleted successfully', 'success');
      
      // Refresh the list
      loadCreditSales();
    } catch (error) {
      console.error('Error deleting credit sale:', error);
      showAlert('Failed to delete credit sale', 'error');
    } finally {
      setIsDeleting(false);
    }
  };

  // Handle print credit sale
  const handlePrint = async (sale: any) => {
    try {
      // Fetch credit sale items
      const itemsResponse = await axios.get(`${API_CREDITSALE_ITEMS}?sale_id=${sale.id}`);
      ...
      console.error('Error printing credit sale:', error);
      showAlert('Failed to print credit sale', 'error');
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <p className="ml-3 text-gray-600">Loading Credit Sales...</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white shadow-md rounded-lg overflow-hidden">
      ...
      <div className="bg-primary text-white px-6 py-4 flex flex-col md:flex-row justify-between items-start md:items-center">
        <h2 className="text-xl font-semibold mb-3 md:mb-0">Credit Sales List</h2>
            ...
            {filteredSales.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-gray-500">
                  {searchQuery ? 'No credit sales match your search criteria' : 'No credit sales found'}
                </td>
              </tr>
            ) : (
