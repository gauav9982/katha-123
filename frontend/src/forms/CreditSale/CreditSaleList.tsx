...
const CreditSaleList: React.FC = () => {
  ...
  const fetchCreditSales = async () => {
    setLoading(true);
    try {
      const res = await axios.get(API_URL.CREDIT_SALES);
      setCreditSales(res.data);
    } catch (error) {
      showAlert('Failed to load credit sales', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${API_URL.CREDIT_SALES}/${selectedSale.id}`);
  ...
  const handlePrint = async (sale: any) => {
    try {
      ...
      const itemsResponse = await axios.get(`${API_URL.CREDIT_SALE_ITEMS}?sale_id=${sale.id}`);
