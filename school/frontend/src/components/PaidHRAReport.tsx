...
const PaidHRAReport: React.FC = () => {
  ...
  const calculateReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4009/api/reports/paid-hra/calculate?cacheBuster=true${cacheBuster}`, {
        method: 'POST',
        headers: {
  ...
  const loadReport = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:4009/api/reports/paid-hra/${selectedTeacher}`);
      const result = await response.json();
