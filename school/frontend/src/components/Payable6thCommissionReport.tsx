...
const Payable6thCommissionReport: React.FC = () => {
  ...
  const loadExistingReport = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:4009/api/reports/payable-6th-commission/${selectedTeacher}`);
      const result = await response.json();
  ...
  const calculateReport = async () => {
    ...
    try {
      const response = await fetch('http://localhost:4009/api/reports/payable-6th-commission/calculate', {
        method: 'POST',
        headers: {
