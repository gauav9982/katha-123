...
const SupPaid6thCommissionReport: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedTeacher) {
      const loadExistingReport = async () => {
        const response = await fetch(`http://localhost:4009/api/reports/sup-paid-6th-commission/${selectedTeacher}`);
        const result = await response.json();
  ...
  const calculateReport = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('http://localhost:4009/api/reports/sup-paid-6th-commission/calculate', {
        method: 'POST',
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-2">Support Paid 6th Commission Report</h1>
...
export default SupPaid6thCommissionReport; 