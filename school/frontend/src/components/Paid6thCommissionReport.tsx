...
const Paid6thCommissionReport: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedTeacher) {
      const loadExistingReport = async () => {
        try {
          const response = await fetch(`http://localhost:4009/api/reports/paid-6th-commission/${selectedTeacher}`);
          const result = await response.json();
  ...
  const calculateReport = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('http://localhost:4009/api/reports/paid-6th-commission/calculate', {
        method: 'POST',
