...
const Paid5thCommissionReport: React.FC = () => {
  ...
  const calculateReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4001/api/reports/paid-5th-commission/calculate?cacheBuster=true${cacheBuster}`, {
        method: 'POST',
        headers: {
  ...
  const loadReport = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:4001/api/reports/paid-5th-commission/${selectedTeacher}`);
      const result = await response.json();
          ...
          <div className="mt-6 bg-gray-50 p-4 rounded-lg">
            ...
            <div className="mt-4 p-3 bg-blue-50 rounded border border-blue-200">
              <p className="text-sm text-blue-800">
                <strong>Note:</strong> XBasic àª®àª¾àª‚ yearly increment (150) àª¥àª¾àª¯ àª›à«‡. MBasic àª XBasic àª¨à«‡ follow àª•àª°à«‡ àª›à«‡.
              </p>
            </div>
          </div>
        </>
...
export default Paid5thCommissionReport; 
