...
const Paid5thCommissionReport: React.FC = () => {
  ...
  const calculateReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4009/api/reports/paid-5th-commission/calculate?cacheBuster=true${cacheBuster}`, {
        method: 'POST',
        headers: {
  ...
  const loadReport = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:4009/api/reports/paid-5th-commission/${selectedTeacher}`);
      const result = await response.json();
