...
const Payable7thCommissionReport: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedTeacher) {
      const loadExistingReport = async () => {
        const response = await fetch(`http://localhost:4001/api/reports/payable-7th-commission/${selectedTeacher}`);
        const result = await response.json();
  ...
  const calculateReport = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('http://localhost:4001/api/reports/payable-7th-commission/calculate', {
        method: 'POST',
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-2">Payable 7th Commission Report</h1>
      <p className="text-gray-600 mb-6">Extended report from 7th commission date to 11-10-2022 (or retirement)</p>
      ...
      {selectedTeacher && (
        <div className="mb-6">
          <button onClick={calculateReport} disabled={loading} className="bg-purple-600 text-white px-4 py-2 rounded disabled:bg-gray-400">
            {loading ? 'Calculating...' : 'Calculate 7th Commission Report'}
          </button>
        </div>
      )}
...
export default Payable7thCommissionReport; 
