...
const LWPManagement: React.FC = () => {
  ...
  const fetchLWPRecords = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:4009/api/lwp-records?cityName=${encodeURIComponent(cityName)}`);
      const data = await response.json();
  ...
  const handleAddLWP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:4009/api/lwp-records', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteLWP = async (id: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:4009/api/lwp-records/${id}`, {
        method: 'DELETE',
      });
