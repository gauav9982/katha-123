...
const SupPayable5thCommissionReport: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedTeacher) {
      const loadExistingReport = async () => {
        const response = await fetch(`http://localhost:4001/api/reports/sup-payable-5th-commission/${selectedTeacher}`);
        const result = await response.json();
  ...
  const calculateReport = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('http://localhost:4001/api/reports/sup-payable-5th-commission/calculate', {
        method: 'POST',
