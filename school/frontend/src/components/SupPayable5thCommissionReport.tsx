...
const SupPayable5thCommissionReport: React.FC = () => {
  ...
  useEffect(() => {
    const session = JSON.parse(localStorage.getItem('schoolLoginSession') || '{}');
    if (session.cityName) {
      setCityName(session.cityName);
    } else {
      navigate('/school');
    }
  }, [navigate]);
  ...
  useEffect(() => {
    if (selectedTeacher) {
      const loadExistingReport = async () => {
        const response = await fetch(`http://localhost:4009/api/reports/sup-payable-5th-commission/${selectedTeacher}`);
        const result = await response.json();
  ...
  const calculateReport = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('http://localhost:4009/api/reports/sup-payable-5th-commission/calculate', {
        method: 'POST',
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-2">Support Payable 5th Commission Report</h1>
      <p className="text-gray-600 mb-6">Extended report from 1st higher grade date to 31-03-2009 (ignoring higher grades between 6th commission and 31-03-2009)</p>
      ...
      {selectedTeacher && (
        <div className="mb-6">
          <button onClick={calculateReport} disabled={loading} className="bg-purple-600 text-white px-4 py-2 rounded disabled:bg-gray-400">
            {loading ? 'Calculating...' : 'Calculate Support Report'}
          </button>
        </div>
      )}
          ...
          <div className="mt-8 p-6 bg-gray-50 rounded-lg">
            <h3 className="text-lg font-semibold mb-3">Report Summary</h3>
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
               <div className="text-center"><p className="text-sm">Total Months</p><p className="text-xl font-bold">{reportData.length}</p></div>
               <div className="text-center"><p className="text-sm">Final Basic</p><p className="text-xl font-bold">{Math.round(reportData[reportData.length - 1]?.basic || 0)}</p></div>
               <div className="text-center"><p className="text-sm">Final MBasic</p><p className="text-xl font-bold">{Math.round(reportData[reportData.length - 1]?.mbasic || 0)}</p></div>
               <div className="text-center"><p className="text-sm">Total Amount</p><p className="text-xl font-bold">{Math.round(reportData.reduce((sum, row) => sum + (row.total_amount || row.total || 0), 0))}</p></div>
            </div>
          </div>
        </>
      )}
...
export default SupPayable5thCommissionReport; 