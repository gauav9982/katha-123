...
const SupPayable6thCommissionReport: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedTeacher) {
      const loadExistingReport = async () => {
        const response = await fetch(`http://localhost:4001/api/reports/sup-payable-6th-commission/${selectedTeacher}`);
        const result = await response.json();
  ...
  const calculateReport = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('http://localhost:4001/api/reports/sup-payable-6th-commission/calculate', {
        method: 'POST',
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-2">Support Payable 6th Commission Report</h1>
      <p className="text-gray-600 mb-6">Extended report for internal calculations (End Date: 11-10-2022 or retirement)</p>
...
export default SupPayable6thCommissionReport; 
