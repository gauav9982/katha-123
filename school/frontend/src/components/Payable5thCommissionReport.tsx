...
const Payable5thCommissionReport: React.FC = () => {
  ...
  const calculateReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4001/api/reports/payable-5th-commission/calculate?cacheBuster=true${cacheBuster}`, {
        method: 'POST',
        headers: {
  ...
  const loadReport = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:4001/api/reports/payable-5th-commission/${selectedTeacher}`);
      const result = await response.json();
