...
const PayableHRAReport: React.FC = () => {
  ...
  const calculateReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4001/api/reports/payable-hra/calculate?cacheBuster=true${cacheBuster}`, {
        method: 'POST',
        headers: {
  ...
  const loadReport = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:4001/api/reports/payable-hra/${selectedTeacher}`);
      const result = await response.json();
        ...
        <div className="mt-6 p-4 bg-gray-100 rounded-lg">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <span>â‚¹{reportData.reduce((sum, row) => sum + (row.hraAmount || 0), 0).toLocaleString()}</span>
            </div>
            <div>
              <span className="font-medium">Average HRA: </span>
              <span>â‚¹{Math.round(reportData.reduce((sum, row) => sum + (row.hraAmount || 0), 0) / reportData.length).toLocaleString()}</span>
            </div>
            <div>
              <span className="font-medium">Total Basic: </span>
              <span>â‚¹{reportData.reduce((sum, row) => sum + (row.basic || 0), 0).toLocaleString()}</span>
            </div>
          </div>
        </div>
      )}
