function calculatePayable5thCommissionReport(teacher, grades, sixthCommission, startDate, endDate) {
  ...
  while (currentDate <= endDate) {
    ...
    // If this is the last row (March 2009), override mbasic and gradePay from 6th Commission
    const isLastRow = (year === 2009 && month === 3);
    if (isLastRow) {
      currentMBasic = sixthCommission.payable_basic;
      currentGradePay = sixthCommission.payable_grade_pay;
    }
...
function calculatePayableHRAReport(teacher, grades, sixthCommission, startDate, endDate) {
  ...
  // Get data from Support Payable 5th Commission and 6th Commission reports
  ...
  while (currentDate <= endDate) {
    ...
    // Get data from Support Payable 5th Commission first, then 6th Commission
    let mbasic, mgredpay, total;
    const sup5thItem = sup5thMap.get(key);
    const sup6thItem = sup6thMap.get(key);
    
    // Special handling for March 2009 (last row of 5th Commission)
    const isLastRow5thCommission = (year === 2009 && month === 3);
    
    if (isLastRow5thCommission && sup6thItem) {
      // For March 2009, use 6th Commission values
      mbasic = sup6thItem.mbasic;
      mgredpay = sup6thItem.gradePay;
      total = sup6thItem.total;
    } else if (sup5thItem) {
      // For other months in 5th Commission period, use 5th Commission values
      mbasic = sup5thItem.mbasic;
      mgredpay = sup5thItem.gradePay;
      total = sup5thItem.total;
    } else if (sup6thItem) {
      // For 6th Commission period, use 6th Commission values
      mbasic = sup6thItem.mbasic;
      mgredpay = sup6thItem.gradePay;
      total = sup6thItem.total;
    } else {
    ...
    // Calculate Basic and Gredpay based on working days
...
function calculatePaidHRAReport(teacher, grades, sixthCommission, startDate, endDate) {
  ...
  // Get data from Support Paid 5th Commission and 6th Commission reports
  const sup5thData = calculateSupPaid5thCommissionReport(teacher, grades, sixthCommission, startDate, endDate);
  const sup6thData = calculatePaid6thCommissionReport(teacher, grades, sixthCommission, null, startDate, endDate);
  ...
  while (currentDate <= endDate) {
    ...
    // Get data from Support Paid 5th Commission first, then 6th Commission
    let mbasic, mgredpay, total;
    const sup5thItem = sup5thMap.get(key);
    const sup6thItem = sup6thMap.get(key);
...
function calculateAllPayableReport(teacher, grades, sixthCommission, seventhCommission, startDate, endDate) {
  ...
  // Get data from all commission reports
  const sup5thData = calculateSupPayable5thCommissionReport(teacher, grades, sixthCommission, startDate, endDate);
...
function calculateAllPaidReport(teacher, grades, sixthCommission, seventhCommission, startDate, endDate) {
  ...
  const sup5thData = calculateSupPaid5thCommissionReport(teacher, grades, sixthCommission, startDate, endDate);
  const sup6thData = calculatePaid6thCommissionReport(teacher, grades, sixthCommission, seventhCommission, startDate, endDate);
  const sup7thData = calculatePaid7thCommissionReport(teacher, grades, seventhCommission, startDate, endDate);
...
function calculateDifferentSalaryReport(teacher, grades, sixthCommission, seventhCommission, startDate, endDate) {
  const payableReport = calculateAllPayableReport(teacher, grades, sixthCommission, seventhCommission, startDate, endDate);
  const paidReport = calculateAllPaidReport(teacher, grades, sixthCommission, seventhCommission, startDate, endDate);

  const payableMap = new Map(payableReport.map(row => [`${row.year}-${row.month}`, row]));
  const paidMap = new Map(paidReport.map(row => [`${row.year}-${row.month}`, row]));

  const differentReport = [];

  for (const [key, payableRow] of payableMap.entries()) {
    const paidRow = paidMap.get(key) || {};

    const diffBasic = (payableRow.basic || 0) - (paidRow.basic || 0);
    const diffGradePay = (payableRow.gredpay || 0) - (paidRow.gredpay || 0);
    const diffTotal = (payableRow.calculatedTotal || 0) - (paidRow.calculatedTotal || 0);
    const diffDaAmount = (payableRow.daAmount || 0) - (paidRow.daAmount || 0);
    const diffHraAmount = (payableRow.hraAmount || 0) - (paidRow.hraAmount || 0);
    const diffPayableTotal = (payableRow.payableTotal || 0) - (paidRow.paidTotal || 0);

    differentReport.push({
      month: payableRow.month,
      year: payableRow.year,
      date: payableRow.date,
      
      payable_basic: payableRow.basic,
      payable_gredpay: payableRow.gredpay,
      payable_total: payableRow.calculatedTotal,
      payable_da_amount: payableRow.daAmount,
      payable_hra_amount: payableRow.hraAmount,
      payable_final_total: payableRow.payableTotal,
      
      paid_basic: paidRow.basic,
      paid_gredpay: paidRow.gredpay,
      paid_total: paidRow.calculatedTotal,
      paid_da_amount: paidRow.daAmount,
      paid_hra_amount: paidRow.hraAmount,
      paid_final_total: paidRow.paidTotal,

      diff_basic: diffBasic,
      diff_gredpay: diffGradePay,
      diff_total: diffTotal,
      diff_da_amount: diffDaAmount,
      diff_hra_amount: diffHraAmount,
      diff_final_total: diffPayableTotal,
    });
  }
