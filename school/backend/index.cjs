...
app.post('/api/da-percentages', (req, res) => {
  try {
    const { month, year, da_percentage } = req.body;
    
    if (!month || !year || !da_percentage) {
      return res.status(400).json({ success: false, error: 'Month, year, and DA percentage are required' });
    }
    ...
    const existing = db.prepare('SELECT * FROM da_percentages WHERE month = ? AND year = ?').get(month, year);
    if (existing) {
      return res.status(400).json({ success: false, error: 'DA percentage entry already exists for this month and year' });
    }

    const stmt = db.prepare('INSERT INTO da_percentages (month, year, da_percentage) VALUES (?, ?, ?)');
    const result = stmt.run(month, year, da_percentage);
    
    res.json({ success: true, message: 'DA percentage added successfully', id: result.lastInsertRowid });
  } catch (error) {
    console.error('Error adding DA percentage:', error);
    res.status(500).json({ success: false, error: error.message });
  }
...
app.put('/api/da-percentages/:id', (req, res) => {
  try {
    ...
    const existing = db.prepare('SELECT * FROM da_percentages WHERE id = ?').get(id);
    if (!existing) {
      return res.status(404).json({ success: false, error: 'DA percentage entry not found' });
    }

    const stmt = db.prepare('UPDATE da_percentages SET month = ?, year = ?, da_percentage = ? WHERE id = ?');
    stmt.run(month, year, da_percentage, id);
    
    res.json({ success: true, message: 'DA percentage updated successfully' });
  } catch (error) {
    console.error('Error updating DA percentage:', error);
    res.status(500).json({ success: false, error: error.message });
  }
...
app.delete('/api/da-percentages/:id', (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'DA percentage deleted successfully' });
  } catch (error) {
    console.error('Error deleting DA percentage:', error);
    res.status(500).json({ success: false, error: error.message });
  }
